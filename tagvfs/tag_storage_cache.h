...
В случае ошибок элемент не добавляется и блок пользовательских данных очищается.
\param cache кэш-хранилище
...
/*! Блокирует номер ino в кэше у существующего элемента.
Если есть элемент с номером, то удаляет у него имя и пользвоательские данные.
Ошибки: -ENOENT - нет такого элемента
\param cache кэш-хранилище
\param ino номер элемента для блокировки. Элемент с номером должен существовать (иначе вернётся ошибка)
\return отрицательный код ошибки. 0 - ошибок нет */
